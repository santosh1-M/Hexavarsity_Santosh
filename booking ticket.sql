...
select max(capacity) from screen 
where capacity<(select max(capacity) from screen);
select name,capacity from screen 
order by capacity desc limit 1 offset 1;
...
select class_type,max(capacity) as max_capacity 
from screen join seat on screen.screen_id=seat.screen_id group by class_type;
...
from screen inner join seat on screen.screen_id=seat.screen_id 
inner join showseat on seat.seat_id=seat.seat_id;
...
SELECT name, email FROM Customerstable 
...
select class_type,count(*) from screen  group by class_type having count(*)>1;
